...
public class DrawMusic : MonoBehaviour
{
    ...
    private void Start()
    {
        LogController.LogWarning("DrawMusic Child count:" + transform.childCount);
        // initialize cubes
        ...
        // debug
        Thread.Sleep(500);
        LogController.LogWarning("cubes size: " + cubes.Count);

        // log
        LogController.LogWarning("DrawMusic start() invoked.");
    }
    ...
    void Update()
    {
        ...
        float debug_max_height = -99999;
        float debug_min_height = 99999;
        for (int i = 0; i < num_bins; i++)
        {
            ...
            {
                LogController.LogWarning($"Exception: {e.Message}");
                LogController.LogWarning("cubes size:" + cubes.Count + " i:" + i);
            }
        }
